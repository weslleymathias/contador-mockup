...
export function NetworkSettings() {
  ...
  const getSignalIcon = (signal: number) => {
    if (signal >= 70) return <Signal className="w-5 h-5" style={{ color: '#336699' }} />;
    if (signal >= 40) return <Signal className="w-5 h-5 text-yellow-600" />;
  ...
  return (
    <div className="container mx-auto p-4 max-w-4xl">
      <div className="mb-6">
        ...
        <p className="text-gray-600">Gerencie conexões WiFi e modo ponto de acesso</p>
      </div>
          ...
          {networks.map((network) => (
            <div key={network.ssid}>
              ...
              {selectedNetwork === network.ssid && network.secured && !network.connected && (
                <div className="ml-4 mr-4 mt-2 p-4 bg-gray-50 rounded-lg border-2 border-gray-300">
      ...
      <Card className="p-6">
        ...
        <div className="bg-slate-50 border rounded-lg p-4 mb-4" style={{ borderColor: '#336699' }}>
          <p className="text-sm text-slate-700">
            Ative o modo AP para permitir que outros dispositivos se conectem diretamente a este equipamento.
          </p>
        </div>
        ...
        <div className="space-y-4">
          ...
          {apMode && (
            <>
              ...
              <Button className="w-full" onClick={() => toast.success('Configurações do AP salvas!')}>
