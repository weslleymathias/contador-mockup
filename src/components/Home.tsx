...
export function Home() {
  ...
  const addCount = (increment: number) => {
    ...
    // Simular feedback visual de cruzamento
    ...
    <div className="container mx-auto p-4 max-w-7xl">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card className="p-6">
          ...
          {/* Main Counter */}
          <div className="mt-6 bg-gradient-to-br from-slate-50 to-slate-100 rounded-xl p-8 text-center border-2" style={{ borderColor: '#336699' }}>
            <div className="text-gray-600 mb-2">Contagem Total</div>
            <div className="text-8xl tabular-nums tracking-tight" style={{ color: '#002B6B' }}>{count}</div>
        ...
        <Card className="p-6">
          ...
          {partials.length === 0 ? (
            <div className="text-center py-12 text-gray-500">
              ...
              <p className="text-sm">Adicione parciais durante a contagem para acompanhar o progresso</p>
            </div>
          ) : (
            <>
              {/* Summary Cards */}
              <div className="grid grid-cols-3 gap-3 mb-4">
                <div className="bg-blue-50 rounded-lg p-3 border border-blue-200">
                  ...
                  <div className="text-2xl text-blue-900">{partials.length}</div>
                </div>
                <div className="bg-green-50 rounded-lg p-3 border border-green-200">
                  <div className="text-xs text-gray-600">Peso Total</div>
                  <div className="text-2xl text-green-900">{totalWeight.toFixed(1)} kg</div>
                </div>
                <div className="bg-purple-50 rounded-lg p-3 border border-purple-200">
                  <div className="text-xs text-gray-600">Peso Médio</div>
                  <div className="text-2xl text-purple-900">{avgWeight.toFixed(1)} kg</div>
                </div>
              </div>
